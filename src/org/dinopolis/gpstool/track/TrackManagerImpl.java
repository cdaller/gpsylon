 ...
 * {@link org.dinopolis.gpstool.gpsinput.GPSTrack} objects that are added or
 * downloaded are converted to {@link org.dinopolis.gpstool.track.Track}
 * objects, so they may be projected and printed.
 *
...
{
  private int last_unnamed_track_id = 0;
  private NumberFormat unnamed_track_format_ = new DecimalFormat("Track###");
  ...
  public void initialize(PluginSupport support)
  {
    ...
    active_track_identifier_ = resources_
        .getString(KEY_TRACK_ACTIVE_TRACK_IDENTIFIER);
  }
  ...
  // ----------------------------------------------------------------------
  /**
   * Add a track. If a track with the same identifier already exists, the old
   * track is overwritten.
   *
   * @param track
   *          the track to add.
   */
  public void addTrack(GPSTrack track)
  {
    ...
    synchronized (track_map_)
    {
      if (track.getIdentification() == null)
      {
        String id = unnamed_track_format_.format(last_unnamed_track_id++);
        track.setIdentification(id);
      }
      if (Debug.DEBUG)
      ...
      fireTrackChangedEvent(new TrackChangedEvent(this, track.getIdentification(),
  ...
  // ----------------------------------------------------------------------
  /**
   * Returns the track with the given identifier or <code>null</code> if no
   * track was found with the given identifier.
   *
   * @param identifier
   *          the identifier of the track to return.
   * @return the track with the given identifier or <code>null</code> if no
   *         track was found with the given identifier.
   */
  ...
  public void removeTrack(String identifier)
  {
    synchronized (track_map_)
    {
      ...
      fireTrackChangedEvent(new TrackChangedEvent(this, identifier,
  ...
  // ----------------------------------------------------------------------
  /**
   * Defines the track with the given identifier as the active track (the track
   * used by the {@link #addToActiveTrack(GPSTrackpoint)} method).
   *
   * @param identifier
   *          the identifier of the track to use as active track.
   * @throws IllegalArgumentException
   *           if not track exists with the given name or the identifier is null
   */
      ...
      throws IllegalArgumentException
  {
    ...
    if (track == null)
      throw new IllegalArgumentException("Track with identifier '" + identifier
          + "' does not exist.");
  ...
  // ----------------------------------------------------------------------
  /**
   * Returns the name of the active track (the track used by the {@link
   ...
   * @param trackpoint
   *          the trackpoint to add to the track.
   * @throws IllegalArgumentException
   *           if the track does not exist or any of the arguments are null.
   */
   ...
   *
   * @param trackpoint
   *          the trackpoint to add to the active track.
   * @throws IllegalArgumentException
   *           if the track does not exist or is null.
   */
  ...
  // ----------------------------------------------------------------------
  /**
   * Download all tracks from the connected gps device and adds them to the list
   * of tracks.
   *
   * @throws UnsupportedOperationException
   *           if the gps device does not support this operation.
   * @throws GPSException
   *           if the operation threw an exception (e.g. communication problem).
   */
  public void addTracksFromGPSDevice() throws UnsupportedOperationException,
      GPSException
  {
    if (gps_data_processor_ == null)
      throw new UnsupportedOperationException(
          "No gps data processor available (set).");
   ...
   *           if the operation threw an exception (e.g. communication problem).
   */
  public void uploadTracksToGPSDevice() throws UnsupportedOperationException,
      GPSException
  {
    if (gps_data_processor_ == null)
      throw new UnsupportedOperationException(
   ...
   *
   * @param identifiers
   *          the identifiers of the tracks to be uploaded.
   * @throws UnsupportedOperationException
   *           if the gps device does not support this operation.
   * @throws GPSException
   *           if the operation threw an exception (e.g. communication problem).
  ...
  // ----------------------------------------------------------------------
  /**
   * Returns all visible projected tracks (holding the screen coordinates as
   * well as the geographic coordinates) in a List. If no tracks are found (or
   * visible), an empty list is returned. Visiblity is determined for the given
   * projection. The tracks returned are deep cloned, so any changes in these
   * tracks do not influence the tracks stored in the track manager (and vice
   * versa).
   *
   * @param projection
   *          the projection to use.
   * @throws InterruptedException
   *           if the thread was interrupted while calculating the projection.
   *           This happens, if the projection changes again, before the first
   *           calculations are finished and the second SwingWorker interrupts
   *           the first one (the result of the first worker is not valid any
   *           more anyway. This is done in the {@link
      ...
      throws InterruptedException
  {
    ...
    synchronized (track_map_)
    {
      ...
      while (iterator.hasNext())
      {
        ...
        if (Debug.DEBUG)
          Debug.println("trackmanager",
              "getVisibleProjectedTracks. checking track " + id);
        track = (Track) getTrack(id);
        if (isTrackVisible(track, visible_area))
        {
          // System.out.println("projection of track
          ...
          // System.out.println("end of projection of track
          // "+track.getIdentification());
  ...
  // ----------------------------------------------------------------------
  /**
   * Returns a projected track (holding the screen coordinates as well as the
   * geographic coordinates). At the moment, no caching is done, so the
   * calculation is always executed when this method is called. If no track was
   * found with the given identifier, <code>null</code> is returned. The track
   * returned is deep cloned, so any changes in this tracks does not influence
   * the tracks stored in the track manager (and vice versa). TODO: cache calls
  ...
  // ----------------------------------------------------------------------
  /**
   * Returns true if the given track is starts in, ends in or crosses the given
  ...
  public static boolean isTrackVisible(GPSTrack track, GeoExtent visible_area)
  {
    if (Debug.DEBUG && Debug.isEnabled("trackmanager"))
      Debug.println("trackmanager", "isTrackVisible: track="
          + track.getIdentification() + " visible area=" + visible_area
          + " track minlat=" + track.getMinLatitude() + " track maxlat="
          + track.getMaxLatitude() + " track minlon=" + track.getMinLongitude()
    ...
    if (track_size == 1)
      return (visible_area.isInside(start_point.getLatitude(), start_point
          .getLongitude()));

    while (index < track_size)
    {
      next_point = track.getWaypoint(index);
      if (visible_area.intersectsLine(start_point.getLatitude(), start_point
          .getLongitude(), next_point.getLatitude(), next_point.getLongitude()))
        return (true);
