...
public class ModifyTracksLayer extends BasicLayer implements ActionListener, FocusListener, GpsylonKeyConstants
{
  ...
  // ----------------------------------------------------------------------
  /**
   * Initialize with all it needs. TODO: use my own resources
   * 
  ...
  // ----------------------------------------------------------------------
  /**
   * This method is called from a background thread to recalulate the screen coordinates of any
   * geographical objects. This method must store its objects and paint them in the paintComponent()
   * method.
   */
  ...
  public void paintComponent(Graphics g)
  {
    ...
    if (new_track_ != null)
    {
      ...
      if (!trackpoints.isEmpty())
      {
        ...
        while (point_iterator.hasNext())
        {
          ...
          if ((Math.abs(end_x - start_x) > 0.1) || (Math.abs(end_y - start_y) > 0.1))
          {
    ...
    try
    {
      ...
      while (track_iterator.hasNext())
      {
        ...
        /*
         * if((plugin_.isModifyTrackpointsActive()) && (nearest_point_index_ != -1) &&
         * (draw_preview_)) { Trackpoint nearest =
         * (Trackpoint)trackpoints.get(nearest_point_index_); if(nearest_point_index_ >0) previous =
         * (Trackpoint)trackpoints.get(nearest_point_index_-1); if((nearest_point_index_ + 1) <
         * trackpoints.size()) next = (Trackpoint)trackpoints.get(nearest_point_index_+1);
         * 
        ...
        if (point_iterator.hasNext())
        {
          ...
          while (point_iterator.hasNext())
          {
            ...
            if (Debug.DEBUG)
              Debug.println("trackplugin_paint", "painting line: from" + start_x + "/" + start_y + " to " + end_x + "/" + end_y);
  ...
  // ----------------------------------------------------------------------
  /**
   * Focus Listener Method for focus lost. This method checks the validity of the latitude/longitude
   * input fields for valid inputs
   * 
   * @param event the action event
   */
   ...
   * 
   * @param end the start of the mouse drag. If end is null, it is assumed that no drag, but only a
   *          click was performed.
   */
  ...
  // ----------------------------------------------------------------------
  /**
   * Calculates the position of the cross between to trackpoints. The crossposition depends on the
   * position of the trackpoints and on the poistion of the mouse.
   * 
   */
  protected void calcCrossposition()
  {
    try
    {
      ...
      double d = 0, length = 0;
      // ** is used to block the change of the current mouse poistion while calculating the
      // crosspoint **//
  ...
  // ----------------------------------------------------------------------
  /**
   * Calculates and checks the crosspoint. First the crosspoint between the line of the two
   * trackpoints (A and B) with the normal line of it going trough the current mousepoint. Then
   * checks if the crosspoint is valid.
   * 
