...
public class GoogleMapProjection extends Mercator {
	 ...
	 * The name.
	 */
	public final static transient String FlatProjectionName = "CH1903";

	/**
	 * The type of projection.
	 */
	public final static transient int FlatProjectionType = 1903;
	 ...
	 *            height of screen
	 */
	public GoogleMapProjection(LatLonPoint center, float scale, int width,
			int height) {
	...
	public Point forward(float lat, float lon, Point p, boolean isRadian) {
		...
		Point2D result = forward(getCenter().getLatitude(), getCenter()
				.getLongitude(), lat, lon, getScale());
		result.setLocation(result.getX() + getWidth() / 2, -result.getY()
				+ getHeight() / 2);
	...
	static public Point2D forward(float reference_latitude,
			float reference_longitude, float latitude, float longitude,
			float scale) {
		Point2D point = forward(reference_latitude, reference_longitude,
				latitude, longitude);
	...
	static public Point2D ll2lv03(float lat, float lon) {
		double posx, posy;
	...
	static public LatLonPoint lv032ll(int x, int y) {
		...
		lambdap = 2.6779094 + (4.728982 * yp) + (0.791484 * yp * xp)
				+ (0.1306 * yp * (xp * xp)) - (0.0436 * (yp * yp * yp));
		phip = 16.9023892 + (3.238272 * xp) - (0.270978 * (yp * yp))
				- (0.002528 * (xp * xp)) - (0.0447 * (yp * yp) * xp)
				- (0.0140 * (xp * xp * xp));

		LatLonPoint llp = new LatLonPoint((float) phip * 10000 / 3600,
				(float) lambdap * 10000 / 3600);
	...
	static public int lv03tosn(Point2D swiss, int scale) {
		...
		if (scale == 100) {
			sheet = 25 + 5 * (4 - Math.floor((northing - 62000) / 48000))
					+ Math.floor((easting - 480000) / 70000);
		} else if (scale == 50) {
			sheet = 200 + 10 * (9 - Math.floor((northing - 62000) / 24000))
					+ Math.floor((easting - 480000) / 35000);
		} else if (scale == 25) {
			sheet = 600 + 20 * (19 - Math.floor((northing - 302000) / 12000))
					+ Math.floor((easting - 480000) / 17500);
		}
	...
	public LatLonPoint inverse(int x, int y, LatLonPoint llp) {
		...
		LatLonPoint result = inverse(getCenter().getLatitude(), getCenter()
				.getLongitude(), delta_x, delta_y, getScale());
		llp.setLatLon((float) result.getLatitude(), (float) result
				.getLongitude());
		return llp;
	}
	 ...
	 */
	public static LatLonPoint inverse(float reference_latitude,
			float reference_longitude, double px, double py) {
	 ...
	 */
	public static LatLonPoint inverse(float reference_latitude,
			float reference_longitude, double delta_x, double delta_y,
			double scale) {
	...
	public static void main(String[] args) {
		...
		if (args.length > 1 && args.length < 3) {
			...
			if(easting.matches("^......$") && northing.matches("[0123456789].*")){
				...
				//LatLonPoint inpoint = new LatLonPoint(lv032ll(new Integer(northing).intValue(),new Integer(easting).intValue()));
				...
				//LatLonPoint inpoint = new LatLonPoint(lat.degrees(), lon.degrees());
			}
