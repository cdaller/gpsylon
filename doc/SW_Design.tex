...
\item The \interfacename{MouseModePlugin}: GPSMap supports different
...
\item The \interfacename{GUiPlugin} has the possibility to add one or
...
direction, etc.). A \interfacename{GuiPlugin} may use the modules from
...
\item The \classname{LayerPlugin} is the most complete plugin that has
all possiblities of the \interfacename{GuiPlugin} but additionally
represents a layer of the map component. So the location marker layer,
the track layer, all those could be \classname{LayerPlugin}s (and
probably will be one day). GPSMap uses the \classname{MapBean} class
of the openmap library as the central map component. This class uses
\classname{Layer} objects as layers. Unfortunately the openmap
\classname{Layer} is not an interface, but a class, so the
\classname{LayerPlugin} is not an interface, but a class. The layer is
...
background thread. \classname{org.dinopolis.gpstool.gui.BasicLayer}
and \classname{org.dinopolis.gpstool.plugin.BasicLayerPlugin} provide
