 ...
 * @throws IOException if an error occurs during reading.  */
    ...
    throws IOException
  {
    ...
    try
    {
      ...
      {
        linenumber++;
        if(line.startsWith(TrackLayer.TRACK_FORMAT_DEFINITION_IN_FILE_PREFIX))
        {
          line_format =
            new MessageFormat(line.substring(TrackLayer.TRACK_FORMAT_DEFINITION_IN_FILE_PREFIX.length()),
...
//            System.out.println("using format: '"+line.substring(format_def.length())+"'");
        ...
        if(!line.startsWith("#"))
        {
          ...
          TrackPoint info = new TrackPoint(latitude,longitude,altitude,speed,date);
          track.add(info);
        }
      }
      if(Debug.DEBUG)
