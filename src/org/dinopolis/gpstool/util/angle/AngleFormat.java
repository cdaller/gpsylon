 ...
 *
 * This file is taken from the gpstool framework and modified to match
 * the needs.
 *
 ***********************************************************************/
...
public class AngleFormat extends Format {
    ...
    /***
     * Symboles repr?sentant les degrees (0),
     ...
     * enti?res des degrees (0), minutes (1) et secondes (2). Le
     ...
     * (degrees, minutes ou secondes) ? l'int?rieur
        ...
        for (int i=0; i<length; i++) {
            ...
            for (int field=DEGREES_FIELD; field<SYMBOLS.length; field++) {
                if (upperCaseC == SYMBOLS[field]) {
                     ...
                     * comme patron "MM.mm" sans qu'il soit pr?c?d? des degrees.
                    ...
                    if (field!=symbolIndex-1 || parseFinished) {
                        ...
                        widthDecimal=0;
                        decimalSeparator=true;
                        throw new IllegalArgumentException("ERROR: illegal angle pattern_:"+pattern);
                    }
                    if (c==upperCaseC) {
                        /*
                        ...
                        switch (i-startPrefix) {
                            ...
                            default:
                              throw new IllegalArgumentException("ERROR illegal angle pattern:"+pattern);
                        }
                        int w=1; while (++i<length && pattern.charAt(i)==c) w++;
                        widthDecimal=w;
                        parseFinished=true;
    ...
    public synchronized String toPattern() {
        ...
        for (int field=DEGREES_FIELD; field<=SYMBOLS.length; field++) {
            ...
            if (w>0) {
                /*
                 * Proc?de ? l'?criture de la partie enti?re des degrees,
                 * minutes ou secondes. Le suffix du champs pr?c?dent
                 * sera ?crit avant les degrees, minutes ou secondes.
                 */
                 ...
                 * degrees, minutes ou secondes. Le suffix du ce
                                            ...
                                            final FieldPosition pos)
    {
        ...
        if (width1!=0) {
            ...
            if (minutes<0 || minutes>60) {
                ...
                throw new IllegalArgumentException("ERROR: angle overflow: "+angle);
            }
            if (width2!=0) {
        ...
        /*
         * Les variables 'degrees', 'minutes' et 'secondes' contiennent
        ...
        throws IllegalArgumentException
    {
        ...
        throw new IllegalArgumentException("ERROR: not an angle object: "+obj);
    }
     ...
     *
     * @param  angle      Angle ? ?crire, en degrees.
     * @param  toAppendTo Buffer dans lequel ?crire l'angle.
     * @param  pos        En entr?, le code du champs dont on d?sire les index
     *                    ({@link #DEGREES_FIELD},
     *                     {@link #MINUTES_FIELD},
     ...
     *         suffix des minutes et des secondes devront ?tre v?rifi?s avant celui des degrees.
     * @return Le num?ro du champs correspondant au suffix qui vient d'?tre extrait:
     *         -1 pour le pr?fix de l'angle, 0 pour le suffix des degrees, 1 pour le
     ...
     * Les espaces entre les degrees, minutes et secondes sont accept?s. Si l'angle
                                     ...
                                     final boolean spaceAsSeparator)
    {
        ...
        //         'degrees', 'minutes' et 'secondes' les valeurs appropri?es.        //
        ...
        ///////////////////////////////////////////////////////////////////////////////
        {
             ...
             * degrees, minutes ou secondes alors qu'on n'a pas encore lu de nombre,
             ...
             * pr?c?dent le champs des degrees.
             */
            index=pos.getIndex();
            while (index<length && Character.isSpaceChar(source.charAt(index))) index++;
            pos.setIndex(index);
            /*
             * Lit les degrees. Notez que si aucun s?parateur ne s?parait les degrees
...
BigBoss:    switch (skipSuffix(source, pos, DEGREES_FIELD)) {
                /* ----------------------------------------------
                 * ANALYSE DU SYMBOLE SUIVANT LES PR?SUM?S DEGREES
                 * ----------------------------------------------
                 * Les degrees ?taient suivit du pr?fix d'un autre angle. Le pr?fix sera donc
                 ...
                 * ANALYSE DU SYMBOLE SUIVANT LES PR?SUM?S DEGREES
                 * ----------------------------------------------
                 * On a trouv? le symbole des secondes au lieu de celui des degrees. On fait
                 * la correction dans les variables 'degrees' et 'secondes' et on consid?re
                 ...
                 * ANALYSE DU SYMBOLE SUIVANT LES PR?SUM?S DEGREES
                 * ----------------------------------------------
                 * Aucun symbole ne suit les degrees. Des minutes sont-elles attendues?
                 * Si oui, on fera comme si le symbole des degrees avait ?t? l?. Sinon,
                 ...
                 * ANALYSE DU SYMBOLE SUIVANT LES PR?SUM?S DEGREES
                 * ----------------------------------------------
                 * Un symbole des degrees a ?t? explicitement trouv?. Les degrees sont peut-?tre
                ...
                case DEGREES_FIELD: {
                    ...
                    switch (skipSuffix(source, pos, (width1!=0) ? MINUTES_FIELD : PREFIX_FIELD)) {
                         ...
                         * ANALYSE DU SYMBOLE SUIVANT LES PR?SUM?ES MINUTES
                         * ------------------------------------------------
                         * Au lieu des minutes, le symbole lu est celui des degrees. On consid?re
                  ...
                  * ANALYSE DU SYMBOLE SUIVANT LES PR?SUM?S DEGREES
                  * ----------------------------------------------
                  * Un symbole des minutes a ?t? trouv? au lieu du symbole des degrees attendu.
                  * On fera donc la modification dans les variables 'degrees' et 'minutes'. Ces
                 ...
                 case MINUTES_FIELD: {
                     ...
                     switch (skipSuffix(source, pos, (width2!=0) ? MINUTES_FIELD : PREFIX_FIELD)) {
                          ...
                          * ANALYSE DU SYMBOLE SUIVANT LES PR?SUM?ES SECONDES
                          * -------------------------------------------------
                          * Au lieu des degrees, on a trouv? un symbole des minutes ou des
         ...
         //         aurrait ?t? absent, puis calcule l'angle en degrees.    //
         ////////////////////////////////////////////////////////////////////
         if (minutes<0) {
             secondes = -secondes;
         }
         ...
         /*
          * S'il n'y a rien qui permet de s?parer les degrees des minutes (par exemple si
          * le patron est "DDDMMmmm"), alors la variable 'degrees' englobe ? la fois les
          * degrees, les minutes et d'?ventuelles secondes. On applique une correction ici.
          */
         ...
         throws ParseException
     {
         ...
         for (int index=origin; index<length; index++) {
             if (!Character.isWhitespace(source.charAt(index))) {
                 ...
                 throw new ParseException("ERROR:angle parse exception: "+ source +"-" +
     ...
     public synchronized boolean equals(final Object obj) {
         ...
         if (obj!=null && getClass().equals(obj.getClass())) {
             ...
             return width0           == cast.width0            &&
                    ...
                    prefix.equals(cast.prefix )  &&
                    suffix0.equals(cast.suffix0)  &&
                    suffix1.equals(cast.suffix1)  &&
                    suffix2.equals(cast.suffix2)  &&
                    numberFormat.getDecimalFormatSymbols().equals(cast.numberFormat.getDecimalFormatSymbols());
         } else {
             return false;
         }
     }
